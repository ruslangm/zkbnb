...
func (m *defaultL2NftModel) BatchInsertOrUpdate(nfts []*L2Nft) (err error) {
	dbTx := m.DB.Table(m.table).Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"creator_account_index", "owner_account_index", "nft_content_hash", "creator_treasury_rate", "collection_id", "l2_block_height", "ipns_name", "ipns_id", "metadata"}),
	}).Create(&nfts)
	if dbTx.Error != nil {
		return dbTx.Error
	}
...
func (m *defaultL2NftModel) UpdateByIndexInTransact(tx *gorm.DB, l2nft *L2Nft) error {
	dbTx := tx.Model(&L2Nft{}).Select("creator_account_index", "owner_account_index", "nft_content_hash", "creator_treasury_rate", "collection_id", "l2_block_height", "ipns_name", "ipns_id", "metadata").Where("nft_index = ?", l2nft.NftIndex).Updates(map[string]interface{}{
